...
dm.network.dea.pmsg <-
  function(xdata.s1, ydata.s1 = NULL, zdata, xdata.s2 = NULL, ydata.s2, 
    ...
    res.p      <- res.z.shift <- array(NA, c(n, p))
    res.shift  <- array(NA, c(n, p)) 
      ...
      if(leader == "1st"){
        ...
        for(k in o){
          ...
          if(orientation == "i"){
            ...
            add.constraint(lp.ndea, c(-xdata.s1[k,] * res.eff.s1[k,],
                                      ydata.s1[k,],
                                      if(pm == TRUE & k %in% id.pmsg) rep(1, p) else zdata[k,],
                                      -1),
                           indices = c(id.v.s1,
                                       if(is.null(ydata.s1)) NULL else id.u.s1,
                                       if(pm == TRUE & k %in% id.pmsg) id.a else id.p,
                                       id.w.s1), "=", 0)
            
            # CCT
            if(pm == TRUE & k %in% id.pmsg & is.null(xdata.s2)){
        ...
        id.pmsg    <- which(apply(res.s1$x[, (m.s2 + 1):(m.s2 + p), drop = F], 1, sum) == 0)
    ...
    results <- list(eff.s1 = res.eff.s1, eff.s2 = res.eff.s2, 
                    ...
                    v.s2 = res.v.s2, u.s2 = res.u.s2, w.s2 = res.w.s2, z.shift = res.z.shift)
